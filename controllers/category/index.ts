...
const getRequestHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  const { parent, child, parentId, childLimit, exclude, categoryId } = req.query;
  const search = req.query.search as string;

  const options: Prisma.CategoryFindManyArgs = {};

  // Handle direct category ID fetch
  if (categoryId) {
    // If a specific category ID is requested, return that category
    ...
    if (!category) {
      return generateResponse("404", "Category not found.", res, { data: null });
    }
    
    return generateResponse("200", "Category fetched.", res, { data: [category] });
  }

  // Handle parent categories (top-level)
  if (parent) {
    options.where = {
      parentId: {
        isSet: false,
