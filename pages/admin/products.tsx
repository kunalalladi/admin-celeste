...
const Product = () => {
  ...
  useEffect(() => {
    getProducts("?select=id&select=title&select=price&select=images&select=quantity&select=categoryIds", undefined);
  }, [getProducts]);
            ...
            {productsData.map((product, index) => {
              ...
              return (
                <ListRow key={index} ref={isLastRow ? loader : null} className={`justify-between ${isLastRow ? "border-b-0" : ""}`}>
                  ...
                  <ListItem isImage={true} imagePath={product.images[0]?.url || product.images[0]?.src || ""} className="w-16 h-16" />
